...
struct sniff_ip {
        ...
        #define IP_FLAGS   (IP_RF|IP_DF|IP_MF|IP_OFFMASK)
...
if (ntohs (eptr->ether_type) == ETHERTYPE_IP)
{
   ...
   printf("\nHeader length : %d" ,ip->ip_vhl);
   printf("\nTotal length : %d" ,ip->ip_len);
   printf("\nType of service : %d" ,ntohs(ip->ip_tos));
   printf("\nIdentification no : %d" ,ntohs(ip->ip_id));
   printf("\nTime to LIVE: %d" ,ntohs(ip->ip_ttl));
   printf("\nProtocol : %d" ,ntohs(ip->ip_p));
   printf("\nChecksum : %d" ,ntohs(ip->ip_sum));
   printf("\nFlags which are set\n");

   
   if (ip->ip_flags & IP_RF){
   printf(" IP_RF");
   }
   if (ip->ip_flags & IP_DF){
   printf(" IP_DF");
   }
   if (ip->ip_flags & IP_MF){
   printf(" IP_MF");
   }
   if (ip->ip_flags & IP_OFFMASK){
   printf(" IP_OFFMASK");
   }
